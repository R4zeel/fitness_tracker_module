...
class Training:
    ...
    def get_mean_speed(self) -> float:
        ...
        return self.get_distance() / (self.duration * self.M_IN_H) * 60
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self):
        return ((self.WEIGHT_MULTIPLIER_1 * self.weight
                 + ((self.get_mean_speed() * self.KMH_INTO_MS) ** 2
                    / (self.height / self.CM_INTO_METERS))
...
class Swimming(Training):
    ...
    def get_spent_calories(self):
        return ((self.get_mean_speed() + self.MEAN_SPEED_MULTIPLIER)
                * self.MEAN_SPEED_MULTIPLIER_2 * self.weight * self.duration)
